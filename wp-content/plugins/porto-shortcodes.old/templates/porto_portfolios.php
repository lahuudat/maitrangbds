...
if( $load_more_posts && $paged = get_query_var('paged') ){
	$args['paged'] = $paged;
}
...
if ($posts->have_posts()) {
    ...
    <?php if ($view_more) : ?>
        ...
        <div class="<?php if ($portfolio_layout == 'timeline') echo 'm-t-n-xxl'; else echo 'push-top'; ?> m-b-xxl text-center">
            <a class="button<?php echo $view_more_class ? ' ' . str_replace('.', '', $view_more_class) : '' ?>" href="<?php echo get_post_type_archive_link( 'portfolio' ) ?>"><?php _e("View More", 'porto-shortcodes') ?></a>
        </div>
    <?php endif; ?>
